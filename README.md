...
- **이원화의 진짜 경계선은 "재학습 vs 추론"이다 — "분석 vs 예측"이 아니다**
  ...
  - **추론(`churn_prediction/predict.py`, 자유 주기)**: 학습 과정이 전혀 없으므로(`model.pkl`을 그대로 불러와 `predict_proba`만 호출) 데이터 양·시점과 무관하게 항상 안정적으로 동작한다. 정해진 주기 없이 필요할 때마다(매일, 즉석 조회, 특정 구간만 골라보기 등) 자유롭게 실행 가능 — "최신성을 반영한다"는 역할은 재학습이 아니라 이 추론 단계가 담당한다.
...
**VSCode에서 버튼으로 실행**: `app.py`, `train.py`, `predict.py` 모두 기본 경로(`data/WA_FnUseC_TelcoCustomerChurn.csv`, `segment_discovery/outputs/segment_rules.json`)가 코드에 들어가 있어, 명령줄 인자 없이 파일을 열고 우측 상단 ▶(Run Python File) 버튼만 눌러도 그대로 실행됩니다. `predict.py`만 추론 대상 파일(`data/new_customers.csv`)이 미리 있어야 하고, 없으면 안내 메시지를 보여주고 종료합니다.
